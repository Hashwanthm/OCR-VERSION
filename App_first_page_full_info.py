...
# ======================================================
# FULL FIRST PAGE EXCEL SHEET
# ======================================================

def add_first_page_full_info_sheet(wb, full_text, table_rows):
    """Add a separate sheet with all readable text and table data from page 1."""
    ws = wb.create_sheet("FIRST_PAGE_FULL_INFO")
    ...
    set_cell(ws, "A1", "Full Information Extracted From First Page Only", bold=True, size=12, align="center")
    ws["A1"].fill = header_fill

    set_cell(ws, "A3", "Full Text", bold=True)
    full_text_cell = set_cell(ws, "B3", full_text[:32000], align="left")
    full_text_cell.alignment = Alignment(horizontal="left", vertical="top", wrap_text=True)
    ws.row_dimensions[3].height = 250

    start_row = 6
    set_cell(ws, f"A{start_row}", "Extracted Table Rows", bold=True)
    ...
    if table_rows:
        ...
        for row_index, row in enumerate(table_rows, start=start_row + 1):
            for col_index, value in enumerate(row, start=1):
                set_cell(ws, ws.cell(row=row_index, column=col_index).coordinate, value, align="left")

        apply_border(ws, start_row, 1, start_row + len(table_rows), max_cols)
    else:
        set_cell(ws, f"A{start_row + 1}", "No table detected on first page.")
...
def create_form3a_excel(details, monthly_rows, excel_path, full_text="", table_rows=None):
    ...
    add_first_page_full_info_sheet(wb, full_text, table_rows or [])
...
def convert_pdf():
    ...
    try:
        # Extract only the first page. Page index 0 means first page.
        ...
        preview_rows = [
            ...
            ["Factory Address", details.get("factory_address", "")],
            ["Statutory Rate", details.get("statutory_rate", "")],
            ["Date", details.get("date", "")],
            [""],
            ["Page 1 - Monthly Contribution Rows"],
            ["Month", "Wages", "EPF", "EPF 8 1/3", "Pension", "Refund", "Days", "Remarks"],
        ]
        ...
        if monthly_rows:
            ...
            preview_rows.append(["No monthly contribution table detected on the first page."])

        preview_rows.extend([
            [""],
            ["Page 1 - Full Extracted Text"],
            [text or "No readable text detected on the first page."],
            [""],
            ["Page 1 - Full Extracted Table Rows"],
        ])

        if table_rows:
            preview_rows.extend(table_rows)
        else:
            preview_rows.append(["No table detected on the first page."])

        return jsonify({
            "message": "First page Excel created successfully",
            "rows": preview_rows,
